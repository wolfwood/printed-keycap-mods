...
if (is_undef(keycap)) {
  let(x_spacing = is_list(grid_spacing) ? grid_spacing.x : grid_spacing, y_spacing = is_list(grid_spacing) ? grid_spacing.y : grid_spacing, stagger = is_undef(grid_stagger) ? 0 : grid_stagger ? y_spacing/2 : 0) {
    if (!index) { // middle
      ...
      translate([x_spacing,stagger,0]) printable(other=true) mirror([1,0,0]) trackpoint_notch(far=false, index=true) CS("R3");
      translate([x_spacing,stagger+y_spacing,0]) printable(other=true) mirror([1,0,0]) trackpoint_notch(far=true, index=true) CS("R2");
    } else { // index
      printable(true) mirror([1,0,0]) trackpoint_notch(far=false, index=true) CS("R3-homing");
      translate([0,y_spacing,0]) printable(other=true) mirror([1,0,0]) trackpoint_notch(far=true, index=true) CS("R2");
